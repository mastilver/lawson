...
export default function (bucket) {
    return {
        defineModel: (type, modelDefinition) => defineModel(bucket, type, modelDefinition)
    };
}

function defineModel(bucket, type, modelDefinition) {
    const viewCreationPromise = createViews(bucket.manager(), type, modelDefinition);
    ...
    return {
        ...
        create: async function (item) {
            ...
            await insertAsync(id, itemToInsert);
            return itemToInsert;
        },
        update: async function (id, item) {
            ...
            await updateAsync(id, itemToUpdate);
            return itemToUpdate;
        },
        ...
        query: async function ({where = {}, limit, offset} = {}) {
            ...
            const queryResults = await Promise.all(fieldNames.map(x => {
                return executeQuery(bucket, type, x, where[x]);
            }));
...
function createViews(bucketManager, type, modelDefinition) {
    const views = {};
    ...
    return pify(bucketManager.upsertDesignDocument.bind(bucketManager))(`lawson_${type}`, {views});
}

async function executeQuery(bucket, type, fieldName, params) {
    const queryAsync = pify(bucket.query.bind(bucket));

    let q = ViewQuery.from(`lawson_${type}`, `by_${fieldName}`);

    q.stale(ViewQuery.Update.BEFORE);
